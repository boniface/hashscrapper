...
import hashscrapper.domain.{Article, Link}
...
class HashscrapperSpec extends Specification {
  ...
  def extract(url: String, charset: Charset = Charsets.UTF_8): Article = {
    ...
    HashScrapper.extract(rawHTML).get
  }

  def check(pageInfo: Article, title: String, processedTitle: String, metaDescription: String, metaKeywords: String,
